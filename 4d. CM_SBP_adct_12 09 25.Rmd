...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
num_sbp_adct <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  # of being in a race/ethnic group
  num_sbp_adct[[i]] <- lm(sbpavg ~ 1, data = data)
  
  # obtaining probability density function
  num_med[[i]] <- dnorm(x = data$sbpavg,
                        mean = predict(num_sbp_adct[[i]]),
                        sd = sd(num_sbp_adct[[i]]$residuals)) %>% # SD of residuals 
    as.data.frame() %>%
  rename(num_med = ".")
}
...
# cleaning up workspace for next section
rm(i, m, data, num_sbp_adct)
```
## Denominator model for mediator
```{r den med, echo = TRUE}
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
den_sbp_adct <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  den_med[[i]] <- dnorm(x = data$sbpavg, 
                        mean = predict(den_sbp_adct[[i]]),
                        sd = sd(den_sbp_adct[[i]]$residuals)) %>% # SD of residuals
    as.data.frame() %>%
    rename(den_med = ".")
}
...
# checking probabilities
str(num_sel_adct)
summary(num_sel_adct[[20]]) # switch index to check all datasets
...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
imp_stacked_adct_sbp <- list() # list for mediator weights 
...
summary(imp_stacked_adct_sbp[[20]]$sw_exp_f) 
describeBy(imp_stacked_adct_sbp[[20]]$sw_exp_f, 
           imp_stacked_adct_sbp[[20]]$ethnicity, 
           mat = TRUE) 

summary(imp_stacked_adct_sbp[[20]]$sw_med_f) 
describeBy(imp_stacked_adct_sbp[[20]]$sw_med_f, 
           imp_stacked_adct_sbp[[20]]$ethnicity, 
           mat = TRUE) 
...
bal.tab(ethnicity ~
          ...
          cdx_cvd +
          cdx_dm,
        data = imp_stacked_adct_sbp[[1]],
        weights = "sw_exp_f",
        which.treat = .all)

exp_cov <- do.call(rbind, imp_stacked_adct_sbp) %>% 
...
love.plot(ethnicity ~
          ...
          var.names = cov_names, 
          which.treat = .all,
          title = "Covariate Balance for SBP Exposure Weights (AD meta-ROI CT)") -> lp_exp_sbp_adct

ggsave("./results/lp_exp_sbp_adct.jpeg",
       dpi = 400,
       plot = lp_exp_sbp_adct)
...
med_cov <- do.call(rbind, imp_stacked_adct_sbp) %>% 
...
love.plot(sbpavg~
          ...
          var.names = cov_names, 
          which.treat = .all,
          title = "Covariate Balance for SBP Mediator Weights (AD meta-ROI CT)") -> lp_med_sbp_adct

ggsave("./results/lp_med_sbp_adct.jpeg",
       dpi = 400,
       plot = lp_med_sbp_adct)

# table of weights per imputed dataset
do.call(rbind, imp_stacked_adct_sbp) %>%
  as.data.frame() %>% 
  group_by(`.imp`) %>% 
  ...
  gt() -> tbl_wgts_adct_sbp

# saving tables
gtsave(tbl_wgts_adct_sbp, file = "./results/tbl_wgts_adct_sbp.docx")

# saving dataset
save(imp_stacked_adct_sbp, file = "./analysis data/imp_stacked_adct_sbp.Rdata")

# cleaning up workspace
rm(i, m, data, imp_stacked_exp, imp_stacked_med, imp_stacked_sel, cov_names, exp_cov, med_cov, lp_exp_sbp_adct, lp_med_sbp_adct)
```

# Estimating TE and CDEs for tau 
## Total effect
For total effect, we can model scale(ctmetaroi_2) as a function of race/ethnicity, weighted by the exposure IPW.
```{r te mttau, echo = TRUE}
# loading stacked imputed data 
load("./analysis data/imp_stacked_adct_sbp.Rdata") 
...
imp_stacked_adct_sbp2 <- list() # for new datasets created in the loop

# For-loop to estimate MSM for total effect of race/ethnicity on adct
for (i in seq_along(1:m)){
  
  # calculate mean SBP in NHW
  imp_stacked_adct_sbp[[i]] %>% group_by(ethnicity) %>%
  ...
  imp_stacked_adct_sbp2[[i]] <- imp_stacked_adct_sbp[[i]] %>% 
  ...
  design_cde_adct <- svydesign(ids = ~1, 
                               weights = ~sw_med_f, 
                               data = imp_stacked_adct_sbp2[[i]])
  
  # estimating CDE of race/ethnicity on AD meta-ROI CT, with avg SBP centered on the mean for NHW 
  cde_sbp_nhw[[i]] <- svyglm(scale(ctmetaroi_2) ~
                               as.factor(ethnicity)*sbp_ref_nhw +
                               as.factor(mriscanner_2),
                            design = design_cde_adct)
  
  # estimating CDE of race/ethnicity on AD meta-ROI CT, with avg SBP centered on the AHA goal of SBP<120
  cde_sbp_aha[[i]] <- svyglm(scale(ctmetaroi_2) ~
...
adct_sbp_res <- bind_rows(te_adct_res,
                           cde_nhw_res,
                           cde_aha_res) %>% 
  gt() %>% 
  tab_row_group(
    label = "Controlled Direct Effects of Race/Ethnicity on AD meta-ROI CT, Setting Mean SBP=120 mmHg",
    rows = 5:6) %>% 
  tab_row_group(
    label = "Controlled Direct Effects of Race/Ethnicity on AD meta-ROI CT, Setting Mean SBP as Mean SBP in NHW",
    rows = 3:4) %>% 
  tab_row_group(
...
save(adct_sbp_res, file = "./results/adct_sbp_res") # saving gt version, in case we want to merge gt
adct_sbp_res %>% gtsave("./results/adct_sbp_res.docx")
```
