...
# set default to raw mean differences for binary variables, standarized mean differences for continuous
...
```
# Selection weights 
This accounts for selection into the imaging cohort, in this case, who got an MRI and had WMHV measured. In theory, this would be the same as the AD-meta ROI CT measure, but we will estimate separately for each outcome because N's are still slightly different even for same imaging modality. 
...
ggsave("./results/lp_exp_wmhv.jpeg",
       dpi = 400,
       plot = lp_exp_wmhv)
...
ggsave("./results/lp_med_wmhv.jpeg",
       dpi = 400,
       plot = lp_med_wmhv)
...
For the CDE, we model logWMHV_2 as a function of the race/ethnicity, hypertension, and their interaction, weighted by the SW as calculated by the cumulative product of both the exposure, mediator, and selection weights. The estimate for the Black and Hispanic reflect the CDE of those exposures, holding HTN =0, in other words, what is the effect of race/ethnicity if we control hypertension. We also observe race/eth*HTN interactions (representing the additional WMHV if folks had HTN). 
