...
load(file = "./analysis data/causmed_imp.Rdata") # m=5, n=3592, v=68
...
For amyloid and tau deposition, I will include APOE4 allele in the exposure (and mediator) model as a confounder of the association between systemic racism and amyloid/tau. 

## Numerator model for exposure
```{r num exp, echo = TRUE}
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
num_race_ab <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  num_race_ab[[i]] <- multinom(as.factor(ethnicity) ~ 1,
                               data = data)
  
  # obtaining fitted values 
  num_race[[i]] <- num_race_ab[[i]]$fitted.values %>%
    as.data.frame() %>%
...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
den_race_ab <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  # of being in a race/ethnic group
  den_race_ab[[i]] <- multinom(as.factor(ethnicity) ~ 
  ...
  # obtaining fitted values 
  den_race[[i]] <- den_race_ab[[i]]$fitted.values %>%
    as.data.frame() %>%
...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
num_htn_ab <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  # multinomial regression to obtain unconditional prob 
  # of being in a race/ethnic group
  num_htn_ab[[i]] <- glm(as.factor(has_htn) ~ 1,
                         family = binomial(link = "logit"),
                         data = data)
  
  # obtaining fitted values 
  num_med[[i]] <- num_htn_ab[[i]]$fitted.values %>%
    as.data.frame() %>%
  rename(num_med = ".")
}
...
# cleaning up workspace for next section
rm(i, m, data, num_htn_ab)
```
## Denominator model for mediator
```{r den med, echo = TRUE}
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
den_htn_ab <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  den_htn_ab[[i]] <- glm(as.factor(has_htn) ~
                           as.factor(ethnicity) +
  ...
  # obtaining fitted values 
  den_med[[i]] <- den_htn_ab[[i]]$fitted.values %>% 
    as.data.frame() %>%
    rename(den_med = ".")
}
...
# Selection weights 
This accounts for selection into the imaging cohort, in this case, who got an PET and had amyloid measured. In theory, this would be the same as the tau-PET measure, but we will estimate separately for each outcome because N's are still different even for same imaging modality. 

## Numerator model for selection
```{r num sel, echo = TRUE}
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
sel_ab <- list() # for fitted values
num_sel_ab <- list() # numerator for selection into the subsample 
...
for (i in seq_along(1:m)){
  ...
  sel_ab[[i]] <- glm(as.factor(hasabv2) ~ 1,
                     family = binomial(link = "logit"),
                     data = data)
  
  # obtaining fitted values 
  num_sel_ab[[i]] <- sel_ab[[i]]$fitted.values %>%
    as.data.frame() %>%
  rename(num_sel = ".")
}
...
# checking probabilities
str(num_sel_ab)
summary(num_sel_ab[[20]]) # switch index to check all datasets
...
m <- max(imp_stacked$".imp") # number of imputed datasets
sel_ab <- list() # for fitted values
den_sel_ab <- list() # numerator for selection into the subsample 
...
for (i in seq_along(1:m)){
  ...
  # logistic regression, prob of being in the WMHV sample 
  sel_ab[[i]] <- glm(as.factor(hasabv2) ~ 
                       as.factor(ethnicity) +
                       age_1 + # using baseline age 
                       as.factor(gender) +
                       # SES variables 
  ...
  # obtaining fitted values 
  den_sel_ab[[i]] <- sel_ab[[i]]$fitted.values %>%
    as.data.frame() %>%
  rename(den_sel = ".")
}
...
# checking probabilities
str(den_sel_ab)
summary(den_sel_ab[[20]]) # switch index to check all datasets
...
# cleaning up work space
rm(i, m, data, num_sel_ab, den_sel_ab)
```
...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
imp_stacked_ab <- list() # list for mediator weights 
...
summary(imp_stacked_ab[[5]]$sw_exp_f) 
describeBy(imp_stacked_ab[[5]]$sw_exp_f, 
           imp_stacked_ab[[5]]$ethnicity, 
           mat = TRUE) 

summary(imp_stacked_ab[[5]]$sw_med_f) 
describeBy(imp_stacked_ab[[5]]$sw_med_f, 
           imp_stacked_ab[[5]]$ethnicity, 
           mat = TRUE) 
...
bal.tab(ethnicity ~
          ...
          cdx_cvd +
          cdx_dm,
        data = imp_stacked_ab[[1]],
        weights = "sw_exp_f",
        which.treat = .all)

exp_cov <- do.call(rbind, imp_stacked_ab) %>% 
...
love.plot(ethnicity ~
          ...
          stars = "raw", 
          which.treat = .all, 
          title = "Covariate Balance for Exposure Weights (Amyloid-PET)") -> lp_exp_ab

ggsave("./results/lp_exp_ab.jpeg",
       dpi = 400,
       plot = lp_exp_ab)
...
med_cov <- do.call(rbind, imp_stacked_ab) %>% 
...
love.plot(has_htn ~
          ...
          stars = "raw", 
          which.treat = .all, 
          title = "Covariate Balance for HTN Mediator Weights (Amyloid-PET)") -> lp_med_ab

ggsave("./results/lp_med_ab.jpeg",
       dpi = 400,
       plot = lp_med_ab)

# table of weights per imputed dataset
do.call(rbind, imp_stacked_ab) %>%
  as.data.frame() %>% 
  group_by(`.imp`) %>% 
...
# saving tables
gtsave(tbl_wgts_ab, file = "./results/tbl_wgts_ab.docx")

# saving dataset for analysis 
save(imp_stacked_ab, file = "./analysis data/imp_stacked_ab.Rdata")

# cleaning up workspace
rm(i, m, data, imp_stacked_exp, imp_stacked_med, imp_stacked_sel, cov_names, data, exp_cov, lp_exp_ab, lp_med_ab, med_cov)
```

# Estimating TE and CDEs for amyloid 
## Total effect
For total effect, we can model scale(absuvr_2) as a function of race/ethnicity, weighted by the exposure IPW.
```{r te wmhv, echo = TRUE}
# loading stacked imputed data 
load("./analysis data/imp_stacked_ab.Rdata") 

# creating survey design in order to use weights
m <- length(imp_stacked_ab) # number of imputed datasets
te_ab <- list() # list for mediator weights 

# For-loop to estimate MSM for total effect of race/ethnicity on amyloid SUVR
for (i in seq_along(1:m)){
  
  # creating survey design to implement weights 
  design_te_ab <- svydesign(ids = ~1, 
                            weights = ~sw_exp_f, 
                            data = imp_stacked_ab[[i]])
  
  # estimating total effect of race/ethnicity on amyloid SUVR
  te_ab[[i]] <- svyglm(scale(absuvr_2) ~
                         as.factor(ethnicity) +
                         as.factor(abscanner_2), # correcting for PET scanner 
                       design = design_te_ab) 
}

# Pooling estimates across imputations 
pool_te_ab <- as.mira(te_ab) %>% pool()

# Making results presentable 
te_ab_res <- summary(pool_te_ab, conf.int = TRUE) %>%
  as.data.frame() %>% 
...
# cleaning up workspace
rm(i, m, data, design_te_ab, te_ab)
```
...
imp_stacked_ab2 <- list() # list for data with has_htn level changed to reference = 1

for (i in seq_along(1:m)){
  
  imp_stacked_ab2[[i]] <- imp_stacked_ab[[i]] %>%
    mutate_at(c("has_htn"), 
              factor, 
              levels = c(1,0)) 
  ...
  cde_htn1[[i]] <- svyglm(scale(absuvr_2) ~
...
# Outputting amyloid results 
```{r res wmhv, echo = TRUE}
ab_res <- bind_rows(te_ab_res,
                      cde_htn0_res,
                      cde_htn1_res) %>% 
  gt() %>% 
  tab_row_group(
    label = "Controlled Direct Effects of Race/Ethnicity on Global Amyloid-PET SUVR, Setting HTN = 1",
    rows = 5:6) %>% 
  tab_row_group(
    label = "Controlled Direct Effects of Race/Ethnicity on Global Amyloid-PET SUVR, Setting HTN = 0",
    rows = 3:4) %>% 
  tab_row_group(
    label = "Total Effects of Race/Ethnicity on Global Amyloid-PET SUVR",
    rows = 1:2) 

ab_res

save(ab_res, file = "./results/ab_res") # saving gt version, in case we want to merge gt
ab_res %>% gtsave("./results/ab_res.docx")
```
