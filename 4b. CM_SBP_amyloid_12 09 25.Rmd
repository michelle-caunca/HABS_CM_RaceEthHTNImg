...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
num_sbp_ab <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  # of being in a race/ethnic group
  num_sbp_ab[[i]] <- lm(sbpavg ~ 1, data = data)
  
  # obtaining probability density function
  num_med[[i]] <- dnorm(x = data$sbpavg,
                        mean = predict(num_sbp_ab[[i]]),
                        sd = sd(num_sbp_ab[[i]]$residuals)) %>% # SD of residuals 
    as.data.frame() %>%
  rename(num_med = ".")
}
...
# cleaning up workspace for next section
rm(i, m, data, num_sbp_ab)
```
## Denominator model for mediator
```{r den med, echo = TRUE}
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
den_sbp_ab <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  den_med[[i]] <- dnorm(x = data$sbpavg, 
                        mean = predict(den_sbp_ab[[i]]),
                        sd = sd(den_sbp_ab[[i]]$residuals)) %>% # SD of residuals
    as.data.frame() %>%
    rename(den_med = ".")
}
...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
imp_stacked_ab_sbp <- list() # list for mediator weights 
...
summary(imp_stacked_ab_sbp[[20]]$sw_exp_f) 
describeBy(imp_stacked_ab_sbp[[20]]$sw_exp_f, 
           imp_stacked_ab_sbp[[20]]$ethnicity, 
           mat = TRUE) 

summary(imp_stacked_ab_sbp[[20]]$sw_med_f) 
describeBy(imp_stacked_ab_sbp[[20]]$sw_med_f, 
           imp_stacked_ab_sbp[[20]]$ethnicity, 
           mat = TRUE) 
...
bal.tab(ethnicity ~
          ...
          cdx_cvd +
          cdx_dm,
        data = imp_stacked_ab_sbp[[20]],
        weights = "sw_exp_f",
        which.treat = .all)

exp_cov <- do.call(rbind, imp_stacked_ab_sbp) %>% 
...
love.plot(ethnicity ~
          ...
          var.names = cov_names,
          which.treat = .all,
          title = "Covariate Balance for SBP Exposure Weights (Amyloid-PET)") -> lp_exp_sbp_ab

ggsave("./results/lp_exp_sbp_ab",
       dpi = 400,
       plot = lp_exp_sbp_ab)
...
med_cov <- do.call(rbind, imp_stacked_ab_sbp) %>% 
...
love.plot(sbpavg ~
          ...
          var.names = cov_names, 
          which.treat = .all,
          title = "Covariate Balance for SBP Mediator Weights (Amyloid-PET)") -> lp_med_sbp_ab

ggsave("./results/lp_med_sbp_ab.jpeg",
       dpi = 400,
       plot = lp_med_sbp_ab)

# table of weights per imputed dataset
do.call(rbind, imp_stacked_ab_sbp) %>%
  as.data.frame() %>% 
  group_by(`.imp`) %>% 
...
# saving tables
gtsave(tbl_wgts_ab_sbp, file = "./results/tbl_wgts_ab_sbp.docx")

# saving datasets
save(imp_stacked_ab_sbp, file = "./analysis data/imp_stacked_ab_sbp.Rdata")

# cleaning up workspace
rm(i, m, data, imp_stacked_exp, imp_stacked_med, imp_stacked_sel, cov_names, data, exp_cov, med_cov, lp_exp_sbp_ab, lp_med_sbp_ab)
```
...
load("./analysis data/imp_stacked_ab_sbp.Rdata") 
...
imp_stacked_ab_sbp2 <- list() # for new datasets created in the loop

# For-loop to estimate MSM for total effect of race/ethnicity on logWMHV
for (i in seq_along(1:m)){
  
  # calculate mean SBP in NHW
  imp_stacked_ab_sbp[[i]] %>% group_by(ethnicity) %>%
  ...
  imp_stacked_ab_sbp2[[i]] <- imp_stacked_ab_sbp[[i]] %>% 
  ...
  design_cde_ab <- svydesign(ids = ~1, 
                             weights = ~sw_med_f, 
                             data = imp_stacked_ab_sbp2[[i]])
  
  # estimating CDE of race/ethnicity on amyloid SUVR, with avg SBP centered on the mean for NHW 
  cde_sbp_nhw[[i]] <- svyglm(scale(absuvr_2) ~
  ...
  cde_sbp_aha[[i]] <- svyglm(scale(absuvr_2) ~
...
ab_sbp_res <- bind_rows(te_ab_res,
                        cde_nhw_res,
                        cde_aha_res) %>% 
  gt() %>% 
  tab_row_group(
    label = "Controlled Direct Effects of Race/Ethnicity on Global Amyloid-PET SUVR, Setting Setting Mean SBP=120 mmHg",
    rows = 5:6) %>% 
  tab_row_group(
    label = "Controlled Direct Effects of Race/Ethnicity on Global Amyloid-PET SUVR, Setting Mean SBP as Mean SBP in NHW",
    rows = 3:4) %>% 
  tab_row_group(
...
save(ab_sbp_res, file = "./results/ab_sbp_res") # saving gt version, in case we want to merge gt
ab_sbp_res %>% gtsave("./results/ab_sbp_res.docx")
```
