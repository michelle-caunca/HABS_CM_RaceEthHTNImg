...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
num_htn_mttau <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  # multinomial regression to obtain unconditional prob 
  # of being in a race/ethnic group
  num_htn_mttau[[i]] <- glm(as.factor(has_htn) ~ 1,
                            family = binomial(link = "logit"),
                            data = data)
  
  # obtaining fitted values 
  num_med[[i]] <- num_htn_mttau[[i]]$fitted.values %>%
    as.data.frame() %>%
  rename(num_med = ".")
}
...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
den_htn_mttau <- list() # for fitted values
...
for (i in seq_along(1:m)){
  ...
  den_htn_mttau[[i]] <- glm(as.factor(has_htn) ~
                            as.factor(ethnicity) +
  ...
  # obtaining fitted values 
  den_med[[i]] <- den_htn_mttau[[i]]$fitted.values %>% 
    as.data.frame() %>%
    rename(den_med = ".")
}
...
# Prepping some lists/variables for for-loop 
m <- max(imp_stacked$".imp") # number of imputed datasets
imp_stacked_mttau <- list() # list for mediator weights 
...
Final exposure weights mean ~ 1, and max ranging in the 5-6s. 

Final mediator weights mean ~ 1, but max ranging from 23-29, will truncate (see above section for edits to code), and will be consistent and do this for both weights.
```{r check IPW all, echo = TRUE}
# switch index to check all datasets
summary(imp_stacked_mttau[[20]]$sw_exp_f) 
describeBy(imp_stacked_mttau[[20]]$sw_exp_f, 
           imp_stacked_mttau[[20]]$ethnicity, 
           mat = TRUE) 

summary(imp_stacked_mttau[[20]]$sw_med_f) 
describeBy(imp_stacked_mttau[[20]]$sw_med_f, 
           imp_stacked_mttau[[20]]$ethnicity, 
           mat = TRUE) 
...
bal.tab(ethnicity ~
          ...
          cdx_cvd +
          cdx_dm,
        data = imp_stacked_mttau[[20]],
        weights = "sw_exp_f",
        which.treat = .all)

exp_cov <- do.call(rbind, imp_stacked_mttau) %>% 
...
love.plot(ethnicity ~
          ...
          stars = "raw", 
          which.treat = .all,
          title = "Covariate Balance for Exposure Weights (MTL Tau-PET)") -> lp_exp_mttau

ggsave("./results/lp_exp_mttau.jpeg",
       dpi = 400,
       plot = lp_exp_mttau)
...
bal.tab(ethnicity ~
          ...
          cdx_cvd +
          cdx_dm,
        data = imp_stacked_mttau[[1]],
        weights = "sw_med_f",
        which.treat = .all)

med_cov <- do.call(rbind, imp_stacked_mttau) %>% 
...
love.plot(has_htn ~
          ...
          stars = "raw", 
          which.treat = .all,
          title = "Covariate Balance for HTN Mediator Weights (MTL Tau-PET)") -> lp_med_mttau

ggsave("./results/lp_med_mttau.jpeg",
       dpi = 400,
       plot = lp_med_mttau)

# table of weights per imputed dataset
do.call(rbind, imp_stacked_mttau) %>%
  as.data.frame() %>% 
  group_by(`.imp`) %>% 
...
# saving tables
gtsave(tbl_wgts_mttau, file = "./results/tbl_wgts_mttau.docx")

# saving dataset for analysis
save(imp_stacked_mttau, file = "./analysis data/imp_stacked_mttau.Rdata")

# cleaning up workspace
rm(i, m, data, imp_stacked_exp, imp_stacked_med, imp_stacked_sel, cov_names, exp_cov, med_cov, lp_exp_mttau, lp_med_mttau)
```
...
load("./analysis data/imp_stacked_mttau.Rdata") 
...
for (i in seq_along(1:m)){
  ...
  cde_htn0[[i]] <- svyglm(scale(taumedtempsuvr_2) ~
                            as.factor(ethnicity)*as.factor(has_htn),
                              #as.factor(tauscanner_2), # correcting for PET scanner
...
# cleaning up workspace
rm(i, m, data, design_cde_mttau)
```

### Setting HTN = 1
```{r htn1}
# Prepping list
m <- length(imp_stacked_mttau) # number of imputed datasets
cde_htn1 <- list() # list for mediator weights, setting HTN=1 
imp_stacked_mttau2 <- list() # list for data with has_htn level changed to reference = 1

for (i in seq_along(1:m)){
  
  imp_stacked_mttau2[[i]] <- imp_stacked_mttau[[i]] %>%
    mutate_at(c("has_htn"), 
              factor, 
              levels = c(1,0)) 
  ...
  design_cde_mttau <- svydesign(ids = ~1, 
                                weights = ~sw_med_f, 
                                data = imp_stacked_mttau2[[i]])
  
  # estimating CDE of race/ethnicity on MTL tau SUVR, where HTN = 1 as the reference level (mediator and exp*med term drop from model when mediator=1)
  cde_htn1[[i]] <- svyglm(scale(taumedtempsuvr_2) ~
...
mttau_res <- bind_rows(te_mttau_res,
                       cde_htn0_res,
                       cde_htn1_res) %>% 
  gt() %>% 
  tab_row_group(
    label = "Controlled Direct Effects of Race/Ethnicity on MTL Tau-PET SUVR, Setting HTN = 1",
    rows = 5:6) %>% 
  tab_row_group(
    label = "Controlled Direct Effects of Race/Ethnicity on MTL Tau-PET SUVR, Setting HTN = 0",
    rows = 3:4) %>% 
  tab_row_group(
...
save(mttau_res, file = "./results/mttau_res") # saving gt version, in case we want to merge gt
mttau_res %>% gtsave("./results/mttau_res.docx")
```
